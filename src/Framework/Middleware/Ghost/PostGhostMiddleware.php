...
class PostGhostMiddleware extends ShieldMiddleware
{
    public function __construct(Container $container)
    {
        ...
        $this->allow((new PrivateAccess())
            ->addCheckpoint((new Checkpoint('reap', 'ghosts'))->create()
                ->strict(true)
                /**
                 * Requirement: userId
                 * Use case: User is only allowed to create their own ghosts
                 */
                ->willExpect((new Before('userId'))
                    ->satisfiedBy(new Match(function (RequirementData $data) {
                        ...
                        return $request->getParam('organisationId');
                    })))));
    }
}
